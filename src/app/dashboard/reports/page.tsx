      ...
      <div className="space-y-6">
        {filtered.map(report => (
          <div key={report.id} className="glass-card rounded-[32px] border border-white/5 overflow-hidden">
            ...
            <div
              ...
              onClick={() => report.status === "Analyzed" && setExpandedId(expandedId === report.id ? null : report.id)}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-6 text-[13px] text-zinc-500 font-medium">
                  ...
                  <span className="hidden sm:inline font-mono text-[11px] text-zinc-600 opacity-60">CID: {report.cid.slice(0, 16)}...</span>
                </div>
              </div>
              <div className="flex items-center gap-6 shrink-0">
                <span className="hidden lg:block text-[11px] font-bold text-zinc-500 bg-white/[0.03] border border-white/5 px-3 py-1.5 rounded-xl uppercase tracking-widest">{report.type}</span>
                ...
                {report.status === "Analyzed" && (
                  <div className={`p-2 rounded-full bg-white/[0.02] border border-white/5 text-zinc-500 transition-transform duration-300 ${expandedId === report.id ? "rotate-180" : ""}`}>
                    <ChevronDown size={18} />
                  </div>
                )}
              </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="flex-1 h-1.5 bg-white/5 rounded-full overflow-hidden">
                          <div className="h-full bg-emerald-500" style={{ width: `${report.confidence}%` }}></div>
                        </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex flex-wrap gap-2 pt-1">
                        {["LIME", "SHAP", "Attention"].map(tag => (
                          <span key={tag} className="px-2 py-0.5 rounded bg-indigo-500/10 border border-indigo-500/10 text-[10px] font-bold text-indigo-400">{tag}</span>
                        ))}
                      </div>
                    </div>
                  </div>
        ...
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <p className="text-zinc-500 max-w-sm mx-auto">We could not find any reports matching your search. Try adjusting your keywords or filters.</p>
          </div>
        )}
      </div>
    </div>
